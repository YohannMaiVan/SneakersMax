...
import mainLogo from '../../assets/sneakersMax.png';
...
const Header = ({ disconnect }) => {
  ...
  return (
    <div className='header'>
      <NavLink to='/'>
        <Home className='header-icon' size={34} />
      </NavLink>
      <div className='header__central'>
        <div className='header__central-title'>
          <NavLink to='/'>
            <img src={mainLogo} style={{ width: '180px', height: '130px' }} />
          </NavLink>
        </div>
      ...
      {panier > 0 ? (
        <ShoppingBag
          ...
          className='header-icon-notEmpty'
          size={34}
        />
      ) : (
      ...
      <button className='header-disconnect' onClick={disconnectOnClick}>
        Déconnexion
      </button>
    </div>
  );
