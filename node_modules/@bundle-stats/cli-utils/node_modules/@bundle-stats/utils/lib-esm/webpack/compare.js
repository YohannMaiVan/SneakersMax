...
const compareAllModules = jobs => {
  const jobsWithAllModules = jobs.map(job => {
    const modulesByChunkId = _selectors.selectors.modules(job);

    const allModules = Object.entries(modulesByChunkId).reduce((agg, [chunkId, {
      modules
    }]) => {
      // Aggregate chunk modules
      const processedModules = Object.entries(modules).reduce((chunkAgg, [moduleName, moduleData]) => {
        const existingModule = chunkAgg[moduleName]; // @NOTE immutable methods will decrease the perfomace by more than 3x
        // eslint-disable-next-line no-param-reassign

        chunkAgg[moduleName] = _objectSpread(_objectSpread(_objectSpread({}, existingModule), moduleData), {}, {
  ...
  return (0, _compareMetrics.compareMetrics)(jobsWithAllModules, _selectors.selectors.allModules, _metrics.METRIC_TYPE_FILE_SIZE);
};
/**
...
const compareBySection = {
  ...
  [_constants.SECTION_WEBPACK_ALL_MODULES]: compareAllModules,
