...
(key, type) => {
  ...
  if (metric && metric.type) {
    return _objectSpread(_objectSpread({}, _metrics.METRIC_TYPES[metric.type]), metric);
  }

  const resolvedType = type || _metrics.METRIC_TYPE_NUMERIC;
  return _objectSpread(_objectSpread({}, _metrics.METRIC_TYPES[resolvedType]), {}, {
    type: resolvedType,
    label: key
  });
};
...
const getMetricRunInfo = (metric, currentValue, baselineValue) => {
  ...
  return _objectSpread(_objectSpread({}, runInfo), {}, {
    delta,
    deltaPercentage,
